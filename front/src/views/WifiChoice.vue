  ...
  <v-container>
    <div align="center" justify="center" class="wifi">
      <v-row justify="center">
        <div
          class="text-lg-h1 text-md-h1 text-sm-h2 text-h3
    "
        >
          IMirror
        </div>
      ...
      <div class="wifi">
        <v-list rounded>
          ...
          <v-list-item-group v-model="selectItem" color="primary">
            <v-list-item v-for="(item, i) in this.item" :key="i">
              <v-list-item-icon>
...
const connectionService = new ConnectionResources();
...
export default {
  ...
  methods: {
    async fetchWifi() {
      this.listWifi = await connectionService.getWifiList();
